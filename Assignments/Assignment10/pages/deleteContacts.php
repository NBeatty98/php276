  ...
  if (isset($_POST['delete'])) {
    if (isset($_POST['chkbx'])) {
      ...
      foreach ($_POST['chkbx'] as $id) {
        ...
        $sql = "DELETE FROM contacts WHERE con_id=:con_id";
  ...
  if (count($records) === 0) {
    ...
    foreach ($records as $row) {
      $output .= "<tr><td>{$row['con_name']}</td>
        ...
        <td>{$row['con_email']}</td>
        <td>{$row['con_dob']}</td>
        <td>{$row['con_contacts']}</td>
        <td>{$row['con_age']}</td>
        <td><input type='checkbox' name='chkbx[]' value='{$row['con_id']}' /></td></tr>";
    }
