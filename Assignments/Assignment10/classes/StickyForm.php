...
class StickyForm extends Validation
{
  ...
  public function validateForm($GlobalPost, $elementsArr)
  {
    foreach ($elementsArr as $k => $v) {
      ...
      /*IF THE TYPE IS SELECT THEN IT CHEDS TO MAKE SURE WHAT WAS SELECTED IS STICY.  IT DOES NOT CHECK ANY VALIDATION ISSUES AS A SELECT BOX TECHNICALLY WILL BE CORRECT. HOWEVER IT COULD BE SET UP TO CHECK FOR CORRECT INPUT */else if ($elementsArr[$k]['type'] === "select") {
        $elementsArr[$k]['selected'] = $GlobalPost[$k];
      }

      /*THIS CHECKS FOR ANY CHECKBOXES THAT ARE CHECKED AND IT ALSO HAS AN OPTIONAL SETTING (REQUIRED OR NOTREQUIRED).  THE CHECKBOXES CAN BE CHECKED OR NOT CHECKED.  IF THE BOXES ARE TO BE CHECKED AND THEY ARE NOT THEN AN ERROR MESSAGE WILL APPEAR.  NOTE THIS ONLY CHECKS IF ALL THE CHECKBOXES ARE LEFT UNCHECKED AND AT LEAST ONE NEEDS TO BE CHECKED. THIS WILL MAKE THE CHECKBOXES STICKY AS WELL*/else if ($elementsArr[$k]['type'] === 'checkbox') {
      ...
      /*THIS CHECKS FOR ANY RADIO BUTTONS THAT ARE SELECTED. IT ALSO WILL CHECK IF A RADION GROUP IS REQUIRED TO BE SELECTED BY CHECKING THE ACTION (REQUIRED OR NOT REQUIRED).  IT ALSO MAKE THE FORM STICKY */else if ($elementsArr[$k]['type'] === 'radio') {
        if ($elementsArr[$k]['action'] == "required" && !isset($GlobalPost[$k])) {
          $elementsArr[$k]['errorOutput'] = $elementsArr[$k]['errorMessage'];
          $elementsArr['masterStatus']['status'] = "error";
        } else {
