...
$elementsArr = [
  ...
  "email" => [
    "errorMessage" => "<span style='color: red; margin-left: 15px;'>Email cannot be blank and must written as a proper email</span>",
    "errorOutput" => "",
    "type" => "text",
    "value" => "nbeatty@admin.com",
    "regex" => "email",
  ],
...
function addData($post)
{
  ...
  if (isset($_POST['login'])) {
    ...
    $sql = "SELECT ad_name, ad_email, ad_password, ad_status FROM admin WHERE ad_email = :ad_email";

    $bindings = array(array(':ad_email', $post['email'], 'str'));
    ...
    if ($data == 'error') {
      ...
      if (count($data) != 0) {
        ...
        $hashed_password = $data[0]['ad_password'];
        // $hashed_password = password_hash($data[0]['ad_password'], PASSWORD_DEFAULT);

        if (password_verify($post['password'], $hashed_password)) {
          session_start();
          $_SESSION['access'] = "accessGranted";
          $_SESSION['status'] = $data[0]['ad_status'];
          $_SESSION['name'] = $data[0]['ad_name'];
